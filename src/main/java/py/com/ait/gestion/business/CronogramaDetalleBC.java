...
@BusinessController
public class CronogramaDetalleBC extends
		DelegateCrud<CronogramaDetalle, Long, CronogramaDetalleDAO> {
	...
	public void registrar(CronogramaDetalle cronogramaDetalle) {
		...
		try {
			audLogDAO.log(null, cronogramaDetalle,
					usuarioDAO.getUsuarioActual(),
					sesionLogDAO.ObtenerIp(usuarioDAO.getUsuarioActual()),
					Definiciones.Operacion.Insert,
					cronogramaDetalle.getCronogramaDetalleId());
		} catch (Exception e) {
	...
	public void editar(CronogramaDetalle cronogramaDetalle) {
		...
		CronogramaDetalle viejo = this.recuperar(cronogramaDetalle
				.getCronogramaDetalleId());
		super.update(cronogramaDetalle);
		try {
			audLogDAO.log(viejo, cronogramaDetalle,
					usuarioDAO.getUsuarioActual(),
					sesionLogDAO.ObtenerIp(usuarioDAO.getUsuarioActual()),
					Definiciones.Operacion.Update,
					cronogramaDetalle.getCronogramaDetalleId());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public void eliminar(Long id) {
		...
		try {
			audLogDAO.log(cronogramaDetalle, null,
					usuarioDAO.getUsuarioActual(),
					sesionLogDAO.ObtenerIp(usuarioDAO.getUsuarioActual()),
					Definiciones.Operacion.Delete,
					cronogramaDetalle.getCronogramaDetalleId());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public CronogramaDetalle getNextCronogramaDetalle(
		...
		return cronogramaDetalleDAO.getNextCronogramaDetalle(cronogramaDetalle,
				respuesta);
	}
}
